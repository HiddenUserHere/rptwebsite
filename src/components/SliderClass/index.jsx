...
const SliderClass = () => {
  ...
  useEffect(() => {
    document.querySelector('.slider-class-box').style.transform = `translateX(-${ slideShow * 100 }vw)`;
  ...
  return (
    <div className="sliderclass-home">
      <div className="slider-class-box">
        { gameClasses.map(({ id, job, poster, desc, weapon, image, video }, index) => (
              ...
              <div className="class-field">
                ...
                <div className="content-class2">
                  ...
                  <div className="dividerclass">
                    <img src="src/assets/img/ornament2.png" alt=""></img>
                  </div>
                  <h3>{ desc }</h3>
                  <div className="video-content">
                    ...
                    <button className="buttonHandleVideo stylebtn"
                            onClick={ () => handlePlayVideo(`video2-${ index }`) }>
                      <img src="src/assets/img/btn_play.png" alt="Play"></img>
                    </button>
                  </div>
                </div>
              </div>
      ...
      <div className="box-list-indicator">
        { gameClasses.map(({ id, job }, index) => (
          <div key={ `indicator-list-${ index }` }
               className={ `indicator-list ${ index === slideShow ? 'select' : '' }` }></div>
        )) }
      </div>

      <div className="btn-handle-slider">
        <button onClick={ handlePreviousSlide }>
          <img src="src/assets/img/left.png" alt="Previous"></img>
        </button>
        <button onClick={ handleNextSlide }>
          <img src="src/assets/img/right.png" alt="Next"></img>
        </button>
      </div>
    </div>
  );
