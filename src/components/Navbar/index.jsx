...
import './styles.css';
import { getUserPanelUrl } from '../../settings.jsx';
...
const Navbar = ({ config }) => {
  ...
  const renderNavbarOptions = (content, name) => {
    if (!content) return;

    return (
      <div className={ name }>
        { content.map(({ isLink, redirect, text, columns, href }, index) => (
          isLink
    ...
    <nav>
      <div className={ `header-box ${ isScrolled && 'scrolled' }` }>
        <div className="desktop-nav">
          <header className="header-main">
            ...
            <div className="options-box">
              { config.leftContent && renderNavbarOptions(config.leftContent, 'left-content-box') }
              { config.rightContent && renderNavbarOptions(config.rightContent, 'right-content-box') }
            </div>
          </header>
                ...
                return (
                  <div key={ `${ colIndex }-column-sub-menu` } className="column-sub-options">
                    { col.options.map(({ isLink, text, redirect }, optionIndex) => isLink && (
                      <Link onClick={ () => setOpenSub(null) } key={ `${ optionIndex }-item-column-sub-menu` }
                            to={ redirect }>
                        <button className="desktop-sub-menu"><p>{ text }</p></button>
                      </Link>
                    )) }
                  </div>
                );
        ...
        <div className="mobile-nav">
          <header>
            <button onClick={ () => setOpenMobile(true) } className="open-mobile-menu">
              <span>Menu</span>
            </button>
            <button onClick={() => window.open(getUserPanelUrl())}>
              <span>Account</span>
            </button>
          </header>
              ...
              { config.leftContent.map(({ isLink, text, columns, redirect }, index) => {
                return (
                  <div key={ `mobile-${ text }-${ index }` } className="mobile-option">
                    ...
                    { isLink
                      ?
                      <Link onClick={ () => (setOpenMobile(false), setMobileIndex(null)) } to={ redirect }>
                        <button className="mobile-sub-option-btn"><span>{ text }</span></button>
                      </Link>
                      : <>
                        <button onClick={ () => setMobileIndex(prev => prev === index ? null : index) }
                                className="sub-option-indicator">
                          <span>{ text }</span>
                            ...
                            columns.map(({ options }) => (
                              options.map((option, indexOption) => {
                                return option.isLink
                                  && <Link onClick={ () => (setOpenMobile(false), setMobileIndex(null)) }
                                           key={ `mobile-redirect-option-${ option.redirect }-${ option.text }-${ indexOption }` }
                                           to={ option.redirect }>
                                    <button className="mobile-sub-menu-option-generic"><span>{ option.text }</span>
                                    </button>
                                  </Link>;
                              })
                            ))
